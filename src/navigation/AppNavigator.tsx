...
const ICONS: Record<string,string> = { Home:'🏠', Stats:'📊', Add:'➕', Budget:'🎯', Profile:'👤' };
  ...
  return (
        ...
        tabBarIcon: ({ focused }) => {
          ...
          return (
            <View style={[s.tabIco, isAdd&&s.tabIcoAdd, isAdd&&focused&&s.tabIcoAddOn]}>
              <Text style={{ fontSize:isAdd?22:18 }}>{ICONS[route.name]}</Text>
            </View>
          );
        },
      })}
    >
      ...
      <Tab.Screen
        ...
        options={{ title:'Hồ sơ', tabBarBadge:unread>0?unread:undefined, tabBarBadgeStyle:{ backgroundColor:COLORS.danger, fontSize:10 } }}
      />
    </Tab.Navigator>
  );
}

// Full stack navigator — all screens available after auth
function MainStack() {
  return (
  ...
  if (state.isLoading) {
    return (
      <View style={s.splash}>
        ...
        <Text style={{ fontSize:22, fontWeight:'700', color:COLORS.dark, marginBottom:8 }}>SpendingApp</Text>
...
const s = StyleSheet.create({
  ...
  tabBar: { backgroundColor:'#fff', borderTopColor:'#eee', borderTopWidth:0.5, height:82, paddingBottom:18, paddingTop:8 },
  tabLbl: { fontSize:10, fontWeight:'500' },
  tabIco: { width:28, height:28, alignItems:'center', justifyContent:'center' },
  tabIcoAdd: { width:50, height:50, borderRadius:25, backgroundColor:COLORS.primaryLight, marginBottom:6 },
